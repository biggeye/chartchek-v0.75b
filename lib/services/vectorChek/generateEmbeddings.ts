...
function extractMetaExport(mdxTree: Root) {
  // Use a more generic type to avoid TypeScript error
  const metaExportNode = mdxTree.children.find((node) => {
    if (node.type !== 'mdxjsEsm') return false;
    
    // We need to check for data and estree properties safely
    const data = (node as any).data;
    const estree = data?.estree;
    if (!estree || !estree.body || !estree.body[0]) return false;
    
    const firstNode = estree.body[0];
    if (firstNode.type !== 'ExportNamedDeclaration') return false;
    
    const declaration = firstNode.declaration;
    if (!declaration || declaration.type !== 'VariableDeclaration') return false;
    
    const declarations = declaration.declarations;
    if (!declarations || !declarations[0]) return false;
    
    const id = declarations[0].id;
    return id.type === 'Identifier' && id.name === 'meta';
  });
  ...
  // Again, use a more generic approach to access data safely
  const nodeData = (metaExportNode as any).data;
  const estree = nodeData?.estree;
  if (!estree || !estree.body || !estree.body[0]) return undefined;
  
  const firstNode = estree.body[0];
  if (firstNode.type !== 'ExportNamedDeclaration') return undefined;
  
  const declaration = firstNode.declaration;
  if (!declaration || declaration.type !== 'VariableDeclaration') return undefined;
  
  const declarations = declaration.declarations;
  if (!declarations || !declarations[0]) return undefined;
  ...
  return getObjectFromExpression(init as ObjectExpression);
}
...
function processMdxForSearch(content: string): ProcessedMdx {
  ...
  // Define the node types to filter
  const mdxNodeTypes = [
    'mdxjsEsm',
    'mdxJsxFlowElement',
    'mdxJsxTextElement',
  ...
  // Remove all MDX elements from markdown with proper type handling
  const mdTree = filter(
    mdxTree,
    (node) => {
      const nodeType = node.type;
      return !mdxNodeTypes.includes(nodeType as any);
    }
  )
...
async function walk(dir: string, parentPath?: string): Promise<WalkEntry[]> {
  ...
  const flattenedFiles = recursiveFiles.reduce<WalkEntry[]>(
...
async function generateEmbeddings() {
  ...
  const shouldRefresh = argv.refresh as boolean
  ...
  for (const embeddingSource of embeddingSources) {
    ...
    try {
      ...
      if (!shouldRefresh && existingPage?.checksum === checksum) {
        const existingParentPage = existingPage?.parentPage as unknown as { id: any; path: any } | null;
