...
import { KnowledgeDocument, KnowledgeDocumentMetadata, Corpus,  } from "@/types/chartChek/knowledgeBase";
...
class GeminiCorpusService {
  ...
  async createCorpus(displayName: string): Promise<Corpus> {
   ...
   console.log('geminicorpusservice: ', headers);
      if (!response.ok) {
  ...
  async queryCorpus(
    ...
    try {
      const response = await fetch(
        `${GEMINI_API_BASE}/${corpusName}:query?key=${this.apiKey}`,
        {
          method: 'POST',
