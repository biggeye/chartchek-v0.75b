 ...
 * @returns Promise resolving to the KIPU credentials or null if not configured
 */
export async function getKipuCredentials(): Promise<KipuCredentials | null> {
  try {
    const supabase = createClient();
    ...
    // If we have a user, try to get their KIPU credentials
    if (user) {
      const credentials = await getUserKipuCredentials(user.id);
 ...
 * @deprecated Use getKipuCredentials() instead
 */
export async function getFacilityKipuCredentials(facilityId: string): Promise<KipuCredentials | null> {
  console.warn('getFacilityKipuCredentials is deprecated. Use getKipuCredentials() instead.');
  return getKipuCredentials();
}
