...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: messageData, error: threadError } = await supabase
      .from('chat_messages')
      .select('*')
    ...
    if (threadError || !messageData) {
      return NextResponse.json(
        { error: 'Thread not found or access denied', code: 'THREAD_ACCESS_DENIED' },
        { status: 404 }
      );
    }

    // Retrieve message using the AI provider's API
